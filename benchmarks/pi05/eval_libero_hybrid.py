...
# _patch_egl_cleanup()  # Jetson-only workaround; leaks render contexts per env.reset -> sim hang at ~trial 19 on x86
...
def run_episode(env, model, task_description, max_steps, replan_steps=5, num_steps_wait=10, obs=None):
    ...
    while t < max_steps + num_steps_wait:
        ...
        globals()["_hb"] = time.time()  # hb-step: progress heartbeat per sim step
...
def eval_single_task(args, task_id):
    ...
    for trial in tqdm.tqdm(range(args.num_trials), desc=f"Task {task_id}"):
        if trial > 0 and trial % 15 == 0:  # env-recycle: sim hangs after ~19 resets of one env instance
            try:
                env.close()
            except Exception:
                pass
            env, task_description = get_libero_env(task, LIBERO_ENV_RESOLUTION, args.seed)
        env.reset()
        obs = env.set_init_state(initial_states[(trial + getattr(args, "trial_offset", 0)) % len(initial_states)])

        t0 = time.perf_counter()
        # HEARTBEAT hard watchdog: env.step spins in native code can ignore SIGALRM;
        # a watcher thread nukes the subtask (parent harness records task failure).
        ...
        def _watch():
            while True:
                time.sleep(20)
                if time.time() - _hb > 150:
                    import os as _os, sys as _sys
                    print("HEARTBEAT STALL: hard-exiting subtask", flush=True)
                    _os._exit(3)
        if not globals().get("_watch_started"):
            globals()["_watch_started"] = True
            _th.Thread(target=_watch, daemon=True).start()
        import signal as _sig
        class _TrialTimeout(Exception):
            pass
        _sig.signal(_sig.SIGALRM, lambda s, f: (_ for _ in ()).throw(_TrialTimeout()))
        _sig.alarm(240)  # watchdog: hung episode -> failure (applied to both stacks)
        try:
            success = run_episode(env, model, task_description, max_steps,
                                  replan_steps=args.replan_steps, obs=obs)
            globals()["_hb"] = time.time()
        except _TrialTimeout:
            logger.warning(f"  Trial {trial}: WATCHDOG TIMEOUT (240s) -> failure")
            success = False
        finally:
            _sig.alarm(0)
